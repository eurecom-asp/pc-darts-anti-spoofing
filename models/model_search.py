...
from func.genotypes import Genotype, PRIMITIVES
...
class MixedOp(nn.Module):
  ...
  def forward(self, x, weights):
    ...
    if temp1.shape[2] == x.shape[2]:
      ...
      # ans = torch.cat([temp1,self.mp(xtemp2)[:,:,:temp1.shape[2],:temp1.shape[3]]], dim=1)
...
class Network(nn.Module):
  ...
  def __init__(self, C, layers, args, num_classes, criterion, front_end, steps=4, multiplier=4, stem_multiplier=3):
    ...
    self.stem0 = nn.Sequential(
      nn.Conv2d(1, C_curr // 2, kernel_size=3, stride=2, padding=1, bias=False),
