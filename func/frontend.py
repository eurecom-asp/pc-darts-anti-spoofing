...
import utils.util_dsp as nii_dsp
import utils.conf as nii_conf
import torchaudio
...
class LFB(torch_nn.Module):
    ...
    def forward(self, x, is_mask):
        ...
        if self.is_cmvn:
            fb_feature = (fb_feature - fb_feature.mean(self.dim, keepdim=True)) / (self.eps + fb_feature.std(self.dim, keepdim=True))
        
        if is_mask:
            fb_feature = self.mask(fb_feature.permute(0, 2, 1)).permute(0, 2, 1)
